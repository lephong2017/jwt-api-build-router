...
export const FieldTypeSelect= (val)=>{
    return  <Select
                ...
                defaultValue={val.fieldType.listOption['listOption'].filter(item=>val.defaultValue.includes(item.id)).map(item=> item.description)}
                filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
            >
                {
                    val.fieldType.listOption['listOption'].map((item,ind)=>{
                        return <Option key={ind} value={item.name}>{item.description}</Option>
                    })
                }
            </Select>
}
